    ...
    async _loadLights() {
        ...
        try {
            ...
            if (Array.isArray(lightsResponse)) {
                ...
                log('error', 'Unexpected response format from lights endpoint:', lightsResponse);
                throw new Error(_('Invalid lights data format from API'));
            }
            
            log('info', `Found ${lightsArray.length} lights:`, lightsArray.map(l => l.name || 'unnamed'));
            ...
            for (const light of lightsArray) {
                ...
                log('info', `Adding light "${name}" (${lightId}) to list, selected: ${isSelected}`);
...
export var GroupsPage = GObject.registerClass({
        ...
        this._refreshButton.connect('clicked', () => {
            try {
                this._loadGroups().catch(error => {
                    ...
                    this._setStatus(_('Failed to load groups. Check connection settings.'), true);
                });
            } catch (error) {
                log('error', 'Error loading groups:', error);
                this._setStatus(_('Failed to load groups. Check connection settings.'), true);
        ...
        showAllButton.connect('clicked', () => {
            try {
                this._toggleAllGroupsVisibility(true).catch(error => {
                    ...
                    this._setStatus(_('Failed to show all groups. Check connection settings.'), true);
                });
            } catch (error) {
                log('error', 'Error showing all groups:', error);
                this._setStatus(_('Failed to show all groups. Check connection settings.'), true);
        ...
        hideAllButton.connect('clicked', () => {
            try {
                this._toggleAllGroupsVisibility(false).catch(error => {
                    ...
                    this._setStatus(_('Failed to hide all groups. Check connection settings.'), true);
                });
            } catch (error) {
                log('error', 'Error hiding all groups:', error);
                this._setStatus(_('Failed to hide all groups. Check connection settings.'), true);
    ...
    async _loadGroups() {
        ...
        try {
            ...
            groups = groups.sort((a, b) => (a.name || '').localeCompare(b.name || '', undefined, {numeric: true, sensitivity: 'base'}));
