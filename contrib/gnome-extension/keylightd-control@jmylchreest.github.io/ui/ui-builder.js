...
export class UIBuilder {
    ...
    _initIconTheme() {
        try {
            ...
            if (!this._iconTheme) {
                try {
                    this._iconTheme = St.ThemeContext.get_for_stage(global.stage).get_theme();
                    log('debug', 'Initialized icon theme from fallback method');
                } catch (error) {
                    log('warn', 'Failed to initialize icon theme from fallback method:', error);
                }
            }

            // Add the extension's icon path if we have a theme
            if (this._iconTheme && this._extensionPath) {
                const actionsPath = this._extensionPath + '/icons/hicolor/scalable/actions/';
                try {
                    ...
                    if (this._iconTheme.append_search_path) {
                        ...
                        log('warn', 'Could not add icon search path - no supported method found');
                    }
                } catch (error) {
    ...
    createControlSection(config) {
        ...
        const powerButton = new St.Button({
            style_class: isOn ? 'keylightd-power-button keylightd-power-on' : 
        ...
        toggle.connect('notify::state', () => {
            ...
            if (section._brightnessRow && !section._brightnessRow.is_finalized) {
                try {
                    ...
                    if (section._brightnessRow._slider && !section._brightnessRow._slider.is_finalized) {
            ...
            if (section._temperatureRow && !section._temperatureRow.is_finalized) {
                try {
                    ...
                    if (section._temperatureRow._slider && !section._temperatureRow._slider.is_finalized) {
    ...
    updateControlSection(section, updates) {
        ...
        // Update brightness
        if (updates.hasOwnProperty('brightness') && 
            ...
            try {
                ...
                // Only update if different to avoid loops
                if (Math.abs(section._brightnessRow._slider.value - normalizedValue) > 0.01) {
                    section._brightnessRow._slider.value = normalizedValue;
                    
                    // Update label if it exists
                    if (section._brightnessRow._valueLabel && !section._brightnessRow._valueLabel.is_finalized) {
                        section._brightnessRow._valueLabel.text = config.format(updates.brightness);
                    }
                }
            } catch (e) {
        ...
        // Update temperature
        if ((updates.hasOwnProperty('temperature') || updates.hasOwnProperty('temperatureK')) && 
            ...
            try {
                ...
                // Only update if different to avoid loops
                if (Math.abs(section._temperatureRow._slider.value - normalizedValue) > 0.01) {
                    section._temperatureRow._slider.value = normalizedValue;
                    
                    // Update label if it exists
                    if (section._temperatureRow._valueLabel && !section._temperatureRow._valueLabel.is_finalized) {
                        section._temperatureRow._valueLabel.text = config.format(tempKelvin);
                    }
                }
            } catch (e) {
        ...
        // Update name
        if (updates.hasOwnProperty('name') && section._titleLabel && !section._titleLabel.is_finalized) {
            try {
