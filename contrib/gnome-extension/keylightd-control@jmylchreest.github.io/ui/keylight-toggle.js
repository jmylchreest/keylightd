...
// Animation constants
const TOGGLE_ANIMATION_TIME = 300; // milliseconds
...
export const KeylightdControlToggle = GObject.registerClass({
    ...
    _init(extension) {
        ...
        // Animation settings
        this._useAnimations = this._settings.get_boolean('use-animations') ?? true;
        ...
        this.connect('clicked', () => {
            if (!this._isToggling) {
                ...
                this._toggleTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, this._debounceTime, () => {
                    this._isToggling = false;
        ...
        this._initIcons(extension);
    }
    
    _initIcons(extension) {
        // No need to set up direct SVG icon paths here as we'll use getLightStateIcon
        log('debug', 'Initialized icons for KeylightdControlToggle');
    }
     ...
     */
    _setupMenu() {
        // Don't set header here as we set it dynamically in _loadControls
        ...
        settingsButton.connect('clicked', () => this._extension._openPreferences());
    ...
    _subscribeToStateEvents() {
        ...
        this._stateSubscriptions.push(
            this._stateManager.subscribe(StateEvents.ERROR, (data) => {
                const anyOnStateChanged = data.isErroring !== this._stateManager.isErroring();
    ...
    _clearControls() {
        ...
        while (child) {
            ...
            if (child._keylightdId && child._keylightdType) {
                existingSections.set(`${child._keylightdType}:${child._keylightdId}`, child);
            }
    ...
    async _loadControls() {
        ...
        if (this.menu) {
            ...
            this.menu.setHeader(this._getIcon(this._lightState), headerTitle, headerSubtitle);
        }
        ...
        try {
            ...
            if (visibleLights.length > 0) {
                ...
                for (const light of visibleLights) {
                    try {
                        ...
                        const lightDetails = await this._lightsController.getLight(light.id);
                        ...
                        if (existingSection && !existingSection.is_finalized) {
                            ...
                            this._uiBuilder.updateControlSection(existingSection, {
                                ...
                                temperatureK: this._uiBuilder._convertDeviceToKelvin(lightDetails.temperature || 200)
                            });
            ...
            const finalHeaderSubtitle = _('Showing %d/%d groups, %d/%d lights').format(
                visibleGroups.length, totalGroups,
                visibleLights.length, totalLights
            );
            ...
            if (this.menu) {
                ...
                this.menu.setHeader(this._getIcon(this._lightState), finalHeaderTitle, finalHeaderSubtitle);
            }
            ...
            this._lightsContainerSignals.push(
                this._lightsContainer.connect('child-added', () => this._recalculateScrollViewHeight())
            );
            this._lightsContainerSignals.push(
                this._lightsContainer.connect('child-removed', () => this._recalculateScrollViewHeight())
            );
    ...
    _recalculateScrollViewHeight() {
        ...
        const getActorHeight = (actor) => {
            ...
            try {
                ...
                if (natHeight > 0) {
                    height = natHeight;
                    log('debug', `Child ${childIndex} Using natural height: ${height} (min: ${minHeight}, nat: ${natHeight})`);
                } else if (minHeight > 0) {
                    height = minHeight;
                    log('debug', `Child ${childIndex} Using minimum height: ${height} (min: ${minHeight}, nat: ${natHeight})`);
                } else {
                    // Fallback to height property
                    height = actor.height || 0;
                    log('debug', `Child ${childIndex} Using direct height property: ${height}`);
                }
            } catch (e) {
        ...
        while (child) {
            if (!child.is_finalized) {
                ...
                const paddingBottom = getNumericStyleProperty(child, 'padding-bottom');
                
                const childTotal = height + marginTop + marginBottom + paddingTop + paddingBottom;
                totalHeight += childTotal;

                log('debug', `Running total: ${totalHeight}; Child ${childIndex} total: ${childTotal} (height: ${height}, margins: ${marginTop + marginBottom}, padding: ${paddingTop + paddingBottom})`);
            } else {
        ...
        if (settingsButton && !settingsButton.is_finalized) {
            ...
            const buttonMarginTop = getNumericStyleProperty(settingsButton, 'margin-top');
            const buttonMarginBottom = getNumericStyleProperty(settingsButton, 'margin-bottom');
            const buttonPaddingTop = getNumericStyleProperty(settingsButton, 'padding-top');
            const buttonPaddingBottom = getNumericStyleProperty(settingsButton, 'padding-bottom');
            
            const buttonTotal = buttonHeight + buttonMarginTop + buttonMarginBottom + 
                              buttonPaddingTop + buttonPaddingBottom;
            log('debug', `Settings button total: ${buttonTotal} (height: ${buttonHeight}, margins: ${buttonMarginTop + buttonMarginBottom}, padding: ${buttonPaddingTop + buttonPaddingBottom})`);
    ...
    _adjustScrollViewHeightWithActual(actualContentHeight) {
        ...
        const workArea = global.display.get_workspace_manager().get_active_workspace().get_work_area_for_monitor(monitor);
    ...
    _updateGroupUI(groupId, updates) {
        const section = this._uiBuilder.getSectionById(this._lightsContainer, groupId, 'group');
        if (section) {
    ...
    _updateLightUI(lightId, updates) {
        const section = this._uiBuilder.getSectionById(this._lightsContainer, lightId, 'light');
        if (section) {
            this._uiBuilder.updateControlSection(section, updates);
        }
    }
        ...
        try {
            ...
            for (const group of allGroups) {
                try {
                    ...
                    if (!groupDetails.lights || groupDetails.lights.length === 0) continue;
                    ...
                    for (const lightId of groupDetails.lights) {
                        const light = lightMap.get(lightId) || await this._lightsController.getLight(lightId);
                        if (!light) continue;
                    ...
                    if (groupState !== null) {
                        const groupSection = this._uiBuilder.getSectionById(this._lightsContainer, group.id, 'group');
                        if (groupSection && !groupSection.is_finalized) {
                            if (groupSection._toggle && !groupSection._toggle.is_finalized) {
                                groupSection._toggle.state = groupState;
                            }
                            
                            if (groupSection._powerButton && !groupSection._powerButton.is_finalized) {
    ...
    async _toggleGroupState(groupId, state) {
        try {
            ...
            try {
                ...
                if (groupDetails && groupDetails.lights && groupDetails.lights.length > 0) {
                    // Update all light controls that may be part of this group
                    for (const lightId of groupDetails.lights) {
                        const lightSection = this._uiBuilder.getSectionById(this._lightsContainer, lightId, 'light');
                        if (lightSection && !lightSection.is_finalized) {
                            // Update toggle state without triggering callbacks
                            if (lightSection._toggle && !lightSection._toggle.is_finalized) {
                                lightSection._toggle.state = state;
                            }
                            
                            // Update power button appearance
                            if (lightSection._powerButton && !lightSection._powerButton.is_finalized) {
                ...
                log('error', `Error updating UI for lights in group ${groupId}:`, err);
            }
    ...
    async _toggleLights() {
        try {
            ...
            if (visibleGroups.length > 0) {
                ...
                for (const group of visibleGroups) {
                    const section = this._uiBuilder.getSectionById(this._lightsContainer, group.id, 'group');
                    if (section && !section.is_finalized) {
                        // Update toggle state
                        if (section._toggle && !section._toggle.is_finalized) {
            ...
            if (visibleLights.length > 0) {
                ...
                for (const light of visibleLights) {
                    const section = this._uiBuilder.getSectionById(this._lightsContainer, light.id, 'light');
                    if (section && !section.is_finalized) {
                        // Update toggle state
                        if (section._toggle && !section._toggle.is_finalized) {
                            section._toggle.state = newState;
    ...
    _animateSection(section, state) {
        ...
        const animationDuration = this._settings.get_int('animation-duration') || 300;
        ...
        if (state) {
            ...
            section.ease({
                ...
                onComplete: () => {
                    ...
                    if (section._brightnessRow && !section._brightnessRow.is_finalized && section._brightnessRow.visible !== true) {
                        section._brightnessRow.visible = true;
                    }
                    if (section._temperatureRow && !section._temperatureRow.is_finalized && section._temperatureRow.visible !== true) {
    ...
    async _updateToggleStateWithData(visibleGroups, visibleLights) {
        try {
            ...
            if (this.menu && this.menu.setHeader) {
                try {
                    const subtitle = this.menu.header && this.menu.header.subtitle ? 
